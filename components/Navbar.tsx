...
const links = [["Projetos", "#portfolio"]];
    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#07080b]/90 backdrop-blur-xl">
      <nav aria-label="Navegação principal" className="mx-auto max-w-[1180px] px-4 sm:px-6">
        <div className="flex h-16 min-w-0 items-center justify-between gap-3">
          ...
          <a className="hidden items-center gap-2 rounded-full border border-white/15 px-4 py-2.5 text-xs font-bold uppercase tracking-[0.16em] text-white transition-colors hover:border-accent hover:text-accent md:flex" href="https://wa.me/5522998183416?text=Olá%2C%20gostaria%20de%20solicitar%20um%20orçamento." target="_blank" rel="noopener noreferrer">
            Contato <span aria-hidden="true">↗</span>
          </a>

          <button className="shrink-0 rounded-full border border-white/15 p-2 text-white md:hidden" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="menu-mobile" aria-label={open ? "Fechar menu" : "Abrir menu"}>
        ...
        {open && (
          <div id="menu-mobile" className="border-t border-white/10 py-3 md:hidden">
            <div className="flex flex-col">
              ...
              <a href="https://wa.me/5522998183416?text=Olá%2C%20gostaria%20de%20solicitar%20um%20orçamento." className="mt-3 rounded-full bg-accent px-4 py-3 text-center text-sm font-bold text-background-dark">
                Contato
              </a>
            </div>
          </div>
